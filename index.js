...
const pacarNumber = ["6289696389730@s.whatsapp.net"]
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('QR code is ready, Scan now..','white'),color('AGUSZ','red'),color('PAKEZ','lime'))
})
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*Hallo* 👋 @${num.split('@')[0]}\nSelamat datang di group *${mdata.subject}*\nYang betah yah sayang 😅\nJangan lupa sv nomor owner ntar di back @${num.split('@')[0]} 😅`
	...
	client.on('message-new', async (mek) => {
		...
		case 'ytmp4':
				...
				if (anu.error) return reply(anu.error)
				teks = `❏ *Title* : ${anu.result.title}\n❏ *Ukuran* : ${anu.result.size}\n\n❏ *Tunggu Bentar Ya tod, Vidoenya Lagi Di Kirim...*`
				thumb = await getBuffer(anu.result.thumbnail)
				client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
		...
		case 'play':   
				...
				if (anu.error) return reply(anu.error)
				infomp3 = `*Lagu Ditemukan APBOT!!!*\nJudul : ${anu.result.title}\nSource : ${anu.result.source}\nUkuran : ${anu.result.size}\n\n*TUNGGU SEBENTAR ..AUDIO SEDANG DIKIRIM MOHON JANGAN SPAM YA SAYANG*`
		...
		case 'daftar':
					...
					if (namaUser.length >= 30) return reply(`Nama kamu kepanjangan sayang 😗`)
		...
		case 'bug':
					...
					reply('Masalah telah di laporkan ke owner APBOT, Laporan palsu atau main² tidak akan ditanggapi.')
					break
		case 'request':
		case 'req':
					...
					reply('REQUEST ANDA TELAH SAMPAI KE OWNER APBOT, Requests palsu atau main² tidak akan ditanggapi.')
					break
		case 'blocklist': 
		...
		case 'tiktokstalk':
				...
				break
		case 'bot':
					client.sendMessage(from, 'Iya tod? \nJangan lupa daftar iya tod ^_^"\nJika sudah silahkan berarti sudah terdaftar tod ^_^"',MessageType.text, { quoted: mek} )
					break
		case 'test':
